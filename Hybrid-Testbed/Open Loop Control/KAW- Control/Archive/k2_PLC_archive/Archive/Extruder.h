...
public:
    ...
    bool setMaxFeedrate(int maxFeedrate);
    
    // Get e motor max feedrate 
    int getMaxFeedrate();

    // Set acceleration
    bool setAcceleration(int accleration);

    // Get accelleration
    int getAcceleration();

    // Set jerk limits
    bool setJerkLimits(int jerkLimits);

    // Get Jerk limits
    int getJerkLimits();

    // Set minimum extruding and travel feedrate
    bool setMinExtrudingAndTravelFeedrate(int minExtrudingAndTravelFeedrate);

    // Get minimum extruding and travel feedrate
    int getMinExtrudingAndTravelFeedrate();

    // Set enable for steppers
    bool setEnableSteppers(int enableSteppers);

    // Get enable for steppers
    bool getEnableSteppers();

    // Set extrusion motor bed leveling tempurature
    bool setEMotorBedLevelTemp(int eMotorBedLevelTemp);

    // Get extrusion motor bed level temperature
    int getEMotorBedLevelTemp();
    ...
    bool setExtruderTemp(int extruderTemp);

    // Get extruder temperature
    int getExtruderTemp();

    // Set e motor spread cycle mode
    bool setEMotorSpreadcycleMode();

    // Extrude a purge line at the start of the print
    bool extrudePurgeLine();
    ...
    bool setExtrudeFlowrate(int flowrate);

    // Get the extruder flow rate 
    int getExtrudeFlowrate();

    // Set the extruder flow rate 
    bool setRetractFlowrate(int flowrate);

    // Get the extruder flow rate 
    int getRetractFlowrate();

    // Set the units to be used
    bool setUnits(char units);

    // Get the units being used
    int getUnits();

    // Extrude filiment at extrude flowrate
    bool extrude();
...
private:
    ...
    bool _enableSteppers;
    int _eMotorBedLevelTemp;
    int _extruderTemp;
    int _flowrate;
    char _units;
};
...
bool init(int slaveID, int controlMode);
