...
bool SL4848CRInit(int slaveID, int SL4848CR_BAUD_RATE, int controlMode, float inputRangeHigh, float inputRangeLow, signed int PVOffset)
{
  ...
  while(!setInputRangeHigh(slaveID, inputRangeHigh));
  while(!setInputRangeLow(slaveID, inputRangeLow));
...
bool ModbusRTUClientInit(int SL4848CR_BAUD_RATE)
{
  ...
  // Error handling 
 if (!ModbusRTUClient.begin(SL4848CR_BAUD_RATE)) {
...
{  Serial.begin(9600);
  //check to ensure valid temperature range
  //if((inputRangeHigh < 0xFFFF) || (inputRangeHigh > getInputRangeLow(slaveID)))
  //{
    // Write new Input Range High Value
    if (!ModbusRTUClient.holdingRegisterWrite(slaveID, INPUT_RANGE_HIGH_ADDR, inputRangeHigh)) 
    {
      // Error handling
      Serial.println("- Failed to write Input Range High Value! "+ slaveID);
      Serial.println(ModbusRTUClient.lastError());
      return false;
    }
    else{
  ...
  //else
  //{
    //Serial.print("- Temperature High out of range!");
  //}
};
...
bool setInputRangeLow(int slaveID, float inputRangeLow)
{
  Serial.begin(9600);
  //check to ensure valid temperature range
  if((inputRangeLow > -200) || (inputRangeLow < getInputRangeHigh(slaveID)))
  {

    // Write new Input Range Low Value
    if (!ModbusRTUClient.holdingRegisterWrite(slaveID, INPUT_RANGE_LOW_ADDR, inputRangeLow)) 
    {
      // Error handling
      Serial.println("- Failed to write Input Range Low Value! "+ slaveID);
      Serial.println(ModbusRTUClient.lastError());
      return false;
    }
    else{
  ...
  {
    Serial.print("- Temperature Low out of range!"+ slaveID);
  }
};
...
{
  Serial.begin(9600);
  float currentInputRangeHigh = 0;
  // Send Reading request over RS485
  if(!ModbusRTUClient.requestFrom(slaveID, HOLDING_REGISTERS, INPUT_RANGE_HIGH_ADDR, 1))
  {
    // Error handling
    Serial.println("- Failed to request Input Range High! "+ slaveID);
    Serial.println(ModbusRTUClient.lastError());
  }
  else
  {
    // Response handler
    currentInputRangeHigh = ModbusRTUClient.read();
  }
  return currentInputRangeHigh;
};
...
{
  Serial.begin(9600);
  float currentInputRangeLow = 0;
  // Send Reading request over RS485
  if(!ModbusRTUClient.requestFrom(slaveID, HOLDING_REGISTERS, INPUT_RANGE_LOW_ADDR, 1))
  {
    // Error handling
    Serial.println("- Failed to request Input Range Low! "+ slaveID);
    Serial.println(ModbusRTUClient.lastError());
  }
  else
  {
    // Response handler
    currentInputRangeLow = ModbusRTUClient.read();
  }
  return currentInputRangeLow;
};
...
bool setDecimalPointDisplaySelection(int slaveID, int type)
{
  Serial.begin(9600);
  // Write new Input Range High Value
    if (!ModbusRTUClient.coilWrite(slaveID, DECIMAL_POINT_DISPLAY_SELECTION_ADDR, type)) 
    {
      // Error handling
...
bool setPVOffset(int slaveID, signed int PVOffset)
{
  Serial.begin(9600);
  // Write new Input Range High Value
    if (!ModbusRTUClient.holdingRegisterWrite(slaveID, PV_OFFSET_ADDR, PVOffset)) 
    {
      // Error handling
      Serial.println("- Failed to write Present Value Offset! "+ slaveID);
      Serial.println(ModbusRTUClient.lastError());
      return false;
    }
    else{
