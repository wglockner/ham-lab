...
def start_processing():
    ...
    parameters = o3d.io.read_pinhole_camera_parameters("camera_view.json")
    ...
    for line in lines:
        if len(line) > 0:
            if axis == 'x':
                ...
                y_vals = np.unique(line[:, 1])
                for y in y_vals:
                    z_vals = line[line[:, 1] == y][:, 2]
                    avg_z = np.mean(z_vals)
                    averaged_points.append([avg_x, y, avg_z])
            elif axis == 'y':
                avg_y = np.mean(line[:, 1])
                x_vals = np.unique(line[:, 0])
                for x in x_vals:
                    z_vals = line[line[:, 0] == x][:, 2]
                    avg_z = np.mean(z_vals)
